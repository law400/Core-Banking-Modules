...
Imports System.Text
Partial Class CustomerService_Lift_Bulk_Lien
    ...
    Protected Sub upload_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles upload.Click
        ...
        Me.Legend1.Visible = False
        Me.Legend2.Visible = False
        Me.Legend3.Visible = False
        Me.Legend4.Visible = False
        Me.Legend5.Visible = False
    End Sub
            ...
            Try
                ...
                If Button1.Text <> "Return" Then
                    ...
                    For Each row As GridViewRow In GridView1.Rows
                        Dim ChkBoxRows As CheckBox = DirectCast(row.FindControl("chkLien"), CheckBox)

                        'For k = 0 To i - 1

                        If ChkBoxRows.Checked Then
                            'Bulksched.Accountnumber = GridView1.Rows(k).Cells(0).Text
                            Bulksched.Accountnumber = row.Cells(0).Text
                            Bulksched.HoldAmount = row.Cells(1).Text
                            Bulksched.Lientype = row.Cells(2).Text
                            Bulksched.Status = "1"
                            Bulksched.userid = Bulksched.userid
                            Bulksched.userid = Bulksched.userid
                    ...
                    Dim insval As String = "declare @retval int,@retmsg varchar(200) exec proc_postBulk_lienLifting '" & Me.txtBatch.Text.Trim & "',@retval output,@retmsg output select @retval,@retmsg"
                    For Each dr As Data.DataRow In con.SqlDs(insval).Tables(0).Rows
                        retVal = dr(0).ToString
                        retMsg = dr(1).ToString
                    Next
    ...
    Private Function InsertBulkScript(ByVal Bulksched As Lst_Schedule) As String
        ...
        sqlstr = sqlstr & smartobj.EncoteWithSingleQuote(Bulksched.Lientype) & ","
    ...
    Protected Sub chkboxSelectAll_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            Dim ChkBoxHeader As CheckBox = DirectCast(GridView1.HeaderRow.FindControl("chkboxSelectAll"), CheckBox)
            For Each row As GridViewRow In GridView1.Rows
                Dim ChkBoxRows As CheckBox = DirectCast(row.FindControl("chkLien"), CheckBox)
