...
#Region "Pageload"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ll.Text = "ACCOUNT CREATION" & Request.QueryString("menu")
        ...
        If Page.IsPostBack = False Then
            ...
            Dim Dstring4 As String = "SELECT cast(ltrim(rtrim(productcode))as varchar) productcode,productname FROM tbl_BankProduct where status=1 and productclass=1 and node_id = " & Session("node_ID") & ""
            ...
            Dim sel As String = "select isnull(cutover,0) from tbl_bank where node_id = " & Session("node_ID") & ""
...
#Region "task"
    ...
    Protected Sub DrpProduct_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DrpProduct.SelectedIndexChanged
        Dim selExist As String = "Exec Proc_ProdDetail '" & Me.DrpProduct.SelectedValue.Trim & "', " & Session("node_ID") & ""
        If con.SqlDs(selExist).Tables(0).Rows.Count > 0 Then
            ...
            End If
            Dim fetchstaff As String = "exec proc_validStaffAcct '" & Me.DrpProduct.SelectedValue.Trim & "', " & Session("node_ID") & ""
    ...
    Protected Sub Btnsubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Btnsubmit.Click
        ...
        Try
            If DrpBranch.SelectedIndex = 0 Then
            ...
            If DrpProduct.SelectedIndex = 0 Then
                smartobj.alertwindow(Me.Page, "Please Select Account Product", "Prime")
                DrpProduct.Focus()
                Exit Sub
            End If
            ...
            'Me.txtdrint.Text = "0.0"
            'Me.txtcrint.Text = "0.0"
            DrpAccountTier.SelectedValue = "001"
            ...
            '"" & smartobj.DecodeCkeckBox(Me.chkview) & "," & CDec(Me.txtodamt.Text.Trim) & ",'" & Me.txtold.Text.Trim & "','" & Session("Userid").Trim & "',NULL,'" & Me.DrpAccountTier.SelectedValue & "',@retVal Output,@retmsg Output, " & Session("node_ID") & " select @retval 'retval' ,@retmsg 'retmsg'"

            Dim insacc As String = "Declare @retval int,@retmsg varchar(100) exec Proc_InsCASAAccount '" & Me.DrpBranch.SelectedValue.Trim & "'," &
            "'" & Me.DrpProduct.SelectedValue.Trim & "','" & Me.txtCustomerid.Text.Trim & "','" & Me.txtacctdesc.Text.Trim & "','" & Me.txtDRint.Text.Trim & "'," &
            "'" & Me.txtCRInt.Text.Trim & "','" & Me.DrpSubBranch.SelectedValue.Trim & "','" & Me.txtsweep.Text.Trim & "','" & Me.txtstaffID.Text.Trim & "'," &
            "" & smartobj.DecodeCkeckBox(Me.chkview) & "," & CDec(Me.txtodamt.Text.Trim) & ",'" & Me.txtold.Text.Trim & "','" & Session("Userid").Trim & "',NULL,@retVal Output,@retmsg Output, " & Session("node_ID") & " select @retval 'retval' ,@retmsg 'retmsg'"
        ...
        Catch ex As Exception
            smartobj.alertwindow(Me.Page, "Error in Creating Account", "Prime")
        End Try
    End Sub
    Sub charg(ByVal custid As String, ByVal prod As String)
        ...
        Dim selacct As String = "Select accountnumber from tbl_casaaccount where customerid='" & custid & "' and productcode='" & prod & "' and node_id = " & Session("node_ID") & ""
        ...
        sqlStr = sqlStr & smartobj.EncoteWithSingleQuote(Format(CDate(Session("sysdate")), "MM-dd-yyyy")) & ","
        sqlStr = sqlStr & "'" & "001" & "',"
        sqlStr = sqlStr & smartobj.EncoteWithSingleQuote("1") & ","
        sqlStr = sqlStr & smartobj.EncoteWithSingleQuote("Sys") & ",@retVal OUTPUT,@retmsg OUTPUT," & Session("node_ID") & " "
    ...
    Protected Sub BtnReset_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnReset.Click
        ...
        Me.txtFullname.Text = Nothing
        Me.txtOthername.Text = Nothing
        Me.DrpSex.SelectedValue = Nothing
        smartobj.ClearWebPage(Me.Page)
        clear()
        ...
        Me.Label10.Text = Nothing
        Dim frt As String = Nothing
        Me.Label8.Text = Nothing
        Me.Label7.Text = Nothing
        Me.Label9.Text = Nothing
        Me.txtodamt.Text = Nothing
...
#Region "acctdoc"
    Sub rebind2()
        qry = "Exec proc_docreq1 '" & Me.DrpProduct.SelectedValue & "','" & Me.txtCustomerid.Text.Trim & "', " & Session("node_ID") & ""
    ...
    Sub rebind1()
        ...
        qry = "Exec proc_docreq2 '" & Me.DrpProduct.SelectedValue & "','" & Me.txtCustomerid.Text.Trim & "', " & Session("node_ID") & ""
    ...
    Protected Sub Btnsave2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Btnsave2.Click
        Try
            For Each gv As GridViewRow In GridView1.Rows
                Dim selectedChkBxItem As System.Web.UI.WebControls.CheckBox = CType(gv.FindControl("Check2"), System.Web.UI.WebControls.CheckBox)
                If selectedChkBxItem.Checked Then
                    chkBox = True
                    gvIDs += CType(gv.FindControl("LblDoc2"), System.Web.UI.WebControls.Label).Text.ToString + ","
            ...
            If chkBox Then
                ...
                Do While position <> 0
                    ...
                    Dim insval As String = "Insert into tbl_accountreq(customerid,reqid,node_id) Values ('" & Me.txtCustomerid.Text.Trim & "','" & lentrimmsg & "'," & Session("node_ID") & ")"
                    con.SqlDs(insval)
    ...
    Protected Sub Btnsave1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Btnsave1.Click
        Try
            For Each gv As GridViewRow In GridView2.Rows
                Dim selectedChkBxItem As System.Web.UI.WebControls.CheckBox = CType(gv.FindControl("Check1"), System.Web.UI.WebControls.CheckBox)
                If selectedChkBxItem.Checked Then
                    chkBox = True
                    gvIDs += CType(gv.FindControl("LblDoc1"), System.Web.UI.WebControls.Label).Text.ToString
                    Dim insval As String = "Delete from tbl_accountreq where reqid='" & gvIDs & "' and accountnumber='" & Me.txtCustomerid.Text.Trim & "' and node_id = " & Session("node_ID") & ""
                    con.SqlDs(insval)
...
#End Region
    ...
    Protected Sub txtsweep_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtsweep.TextChanged
        Try
            Dim selacct As String = "select accountnumber from tbl_casaaccount where accountnumber='" & Me.txtsweep.Text.Trim & "' and status=1 and node_id = " & Session("node_ID") & ""
            If con.SqlDs(selacct).Tables(0).Rows.Count > 0 Then
    ...
    Protected Sub txtstaffID_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtstaffID.TextChanged
        Try
            qry = "exec proc_Officers '" & Me.txtstaffID.Text.Trim & "', " & Session("node_ID") & ""
    ...
    Protected Sub DrpAccountTier_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DrpAccountTier.SelectedIndexChanged
        If Me.DrpAccountTier.SelectedIndex <> 0 Then
            Dim QryString As String = "Select isnull(MaxDepositAmt,0)'MaxDepositAmt',isnull(MaxCummulativeBal,0)'MaxCummulativeBal' from tbl_Account_Tier where Tier_id = '" & Me.DrpAccountTier.SelectedValue & "' and node_id = " & Session("node_ID") & ""
