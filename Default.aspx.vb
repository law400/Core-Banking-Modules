...
#Region "Declaration"
    ...
    'Public Shared Function ValidateCaptcha(ByVal response As String) As Boolean
    '       Dim secret As String = "6Ldl0kwUAAAAANv3F3I5Hz1HAFuiDri8ru4d8mRC"
    '       Dim client = New WebClient()
    '       Dim reply = client.DownloadString(String.Format("https://www.google.com/recaptcha/api/siteverify?secret={0}&response={1}", secret, response))
    '       Dim jsSerializer As JavaScriptSerializer = New JavaScriptSerializer()
    '       Dim result = jsSerializer.DeserializeObject(reply)
    '       Dim obj2 As Dictionary(Of String, Object) = New Dictionary(Of String, Object)()
    '       obj2 = CType((result), Dictionary(Of String, Object))
    '       Dim captchaResponse = obj2("success").ToString()
    '       Dim res As Boolean = Convert.ToBoolean(captchaResponse)
    '       Return res
    '   End Function
...
#Region "Task"
    Protected Sub Login_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        'Dim isCapthcaValid As Boolean = ValidateCaptcha(Request("g-recaptcha-response"))
        Try

            'If isCapthcaValid Then
            ...
            If Me.txtCoopID.Text = "" Then
                ...
                Else
                    ...
                    Using con As New System.Data.SqlClient.SqlConnection(constring)
                        Using cmd As New System.Data.SqlClient.SqlCommand("Proc_Validatelogin", con)
                            cmd.CommandType = CommandType.StoredProcedure
                            cmd.Parameters.AddWithValue("@Userid", Me.txtuserid.Text.Trim)
                            cmd.Parameters.AddWithValue("@Password", password)
                            cmd.Parameters.AddWithValue("@sessionid", Session("AppStatus"))
                            cmd.Parameters.AddWithValue("@ipadd", Session("ipaddress").Trim)
                            cmd.Parameters.AddWithValue("@comp", Session("Computername").Trim)
                            cmd.Parameters.AddWithValue("@machaddress", Session("MacAddress").Trim)
                            cmd.Parameters.AddWithValue("@SessionIDs", Session("SessionID"))
                            ...
                            cmd.Parameters.AddWithValue("@Node_id", TenantId)
                            con.Open()
                            cmd.ExecuteNonQuery()
                            con.Close()
                    ...
                    Dim url As String = "~/Customerservice/CreateCustomer.aspx"
                    FormsAuthentication.SetAuthCookie(txtuserid.Text, False)
                        Response.Redirect(url, False)
            ...
            'Recaptcha.Focus()
            'smartobj.alertwindow(Me.Page, "Invalid CAPTCHA: Please select valid CAPTCHA", "Prime")
            'End If
        Catch ex As Exception
